...
fields. Parse by field name, never by key order. The two lines that are not
events, `protocol_error` and the `approval_mode` acknowledgement, carry no
`session_id`. Types: `token` (text), `thinking`
