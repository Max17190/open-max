...
/// The minimal profile is the measurement shape: four tools behind a persona
/// line, byte-identical across projects. A built-in the profile hides is
/// refused at dispatch like any unknown tool, listing what is available, so
/// the model cannot reach past the profile by guessing a name.
#[test]
fn a_minimal_profile_turn_freezes_four_tools_and_refuses_hidden_builtins() {
    let (project, home) = fresh_dirs("profile-minimal");
    std::fs::write(project.join("AGENTS.md"), "# Rules\nAlways run the tests.").unwrap();
    let script = vec![
        (sse_tool_calls(&[("glob", serde_json::json!({"pattern": "**/*.rs"}))]), true),
        (sse_text("done"), true),
    ];
    let (base_url, requests, _server) = spawn_scripted_server(script);
    write_settings_with_mode(&home, &base_url, "auto");
    ...
    let out = cmd(&project, &home)
        .args(["--trust-project", "--profile", "minimal", "-p", "look around"])
        .output()
        .unwrap();
    assert_eq!(out.status.code(), Some(0), "stderr: {}", String::from_utf8_lossy(&out.stderr));

    let bodies = requests.lock().unwrap().clone();
    assert_eq!(bodies.len(), 2, "one refused call, one answer");
    ...
    let names: Vec<&str> = first["tools"]
        ...
        .collect();
    assert_eq!(names, ["read_file", "write_file", "edit_file", "bash"]);
    ...
    assert_eq!(
        ...
        "the measurement prompt names no path and carries no project text"
    );
    let second: serde_json::Value = serde_json::from_str(&bodies[1]).unwrap();
    let reply = second["messages"]
        .as_array()
        .unwrap()
        .iter()
        .rev()
        .find(|m| m["role"] == "tool")
        .expect("the refused call still gets a tool reply");
    let text = reply["content"].as_str().unwrap();
    assert!(text.contains("unknown tool: glob"), "{text}");
    assert!(text.contains("read_file, write_file, edit_file, bash"), "{text}");
    let _ = std::fs::remove_dir_all(project.parent().unwrap());
}

/// The flag is validated before anything runs, and a resumed session keeps
/// the profile it was created with, so combining the flag with --continue is
/// an error rather than a silent no-op.
#[test]
fn the_profile_flag_rejects_unknown_names_and_continuation() {
    let (project, home) = fresh_dirs("profile-flag");
    write_settings(&home, "http://127.0.0.1:9/v1");
    let out = cmd(&project, &home).args(["--profile", "tiny", "-p", "hi"]).output().unwrap();
    assert_eq!(out.status.code(), Some(2));
    assert!(String::from_utf8_lossy(&out.stderr).contains("unknown profile"));
    let out = cmd(&project, &home)
        .args(["--profile", "minimal", "--continue", "-p", "hi"])
        .output()
        .unwrap();
    assert_eq!(out.status.code(), Some(2));
    assert!(String::from_utf8_lossy(&out.stderr).contains("--profile applies to a new session"));
    // The maintenance operations run no session, so the flag is refused
    // there too rather than accepted and ignored.
    let out = cmd(&project, &home).args(["--check", "--profile", "minimal"]).output().unwrap();
    assert_eq!(out.status.code(), Some(2));
    assert!(String::from_utf8_lossy(&out.stderr).contains("--profile applies to a session run"));
    let _ = std::fs::remove_dir_all(project.parent().unwrap());
}
