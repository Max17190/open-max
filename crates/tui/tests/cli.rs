...
fn a_truncated_stream_never_runs_the_tool_call_it_carried() {
    let (project, home) = fresh_dirs("truncated-call");
    let (base_url, _requests, _server) =
        spawn_scripted_server(vec![(WRITE_CALL_SSE.to_string(), false)]);
...
/// Control for the refusal above: in the same unattended mode, that exact
/// call does run once the stream finishes. Without this, the refusal test
/// could pass for the wrong reason (a gate, or a call nothing recognized).
#[test]
fn a_finished_stream_still_runs_the_same_write_call() {
    let (project, home) = fresh_dirs("finished-call");
